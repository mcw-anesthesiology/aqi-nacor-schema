...
impl WritableSchemaType for EmailNotificationSetType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("EmailNotificationFirstName", &self.email_notification_first_name, writer)?;
		write_value("EmailNotificationLastName", &self.email_notification_last_name, writer)?;
		write_value("EmailNotificationAddress", &self.email_notification_address.value(), writer)?;
...
impl WritableSchemaType for DemographicType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref dob) = self.dob {
			write_value("DOB", &dob.format("%F").to_string(), writer)?;
		}
...
impl WritableSchemaType for ProcedureType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref proc_start_time) = self.proc_start_time {
			write_value("ProcStartTime", &proc_start_time.to_rfc3339(), writer)?;
		}

		if let Some(ref proc_end_time) = self.proc_end_time {
			write_value("ProcEndTime", &proc_end_time.to_rfc3339(), writer)?;
		}
		...
		if let Some(ref admission_date) = self.admission_date {
			write_value("AdmissionDate", &admission_date.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for ProcedureLocationType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("LocationType", &self.location_type.value().to_string(), writer)?;
...
impl WritableSchemaType for AnesthesiaCaseType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		self.anesthesia_staff_set.write("AnesthesiaStaffSet", writer)?;

		if let Some(ref monitoring_set) = self.monitoring_set {
			monitoring_set.write("MonitoringSet", writer)?;
		}

		self.anesthesia_method_set.write("AnesthesiaMethodSet", writer)?;
...
impl WritableSchemaType for AnesthesiaStaffType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("ProviderCredentials", self.provider_credentials.value(), writer)?;

		if let Some(ref staff_sign_in) = self.staff_sign_in {
			write_value("StaffSignIn", &staff_sign_in.to_rfc3339(), writer)?;
		}

		if let Some(ref staff_sign_out) = self.staff_sign_out {
			write_value("StaffSignOut", &staff_sign_out.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for AnesthesiaMethodType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("AnesthesiaCategory", self.anesthesia_category.value(), writer)?;

		if let Some(ref anesthesia_subcategory) = self.anesthesia_subcategory {
			write_value("AnesthesiaSubCategory", anesthesia_subcategory.value(), writer)?;
		}

		write_value("AnesthesiaStartTime", &self.anesthesia_start_time.to_rfc3339(), writer)?;
		write_value("AnesthesiaEndTime", &self.anesthesia_end_time.to_rfc3339(), writer)?;
		...
		if let Some(ref anesthesia_induction_start_time) = self.anesthesia_induction_start_time {
			write_value("AnesthesiaInductionStartTime", &anesthesia_induction_start_time.to_rfc3339(), writer)?;
		}

		if let Some(ref anesthesia_maintenance) = self.anesthesia_maintenance {
			write_value("AnesthesiaMaintenance", anesthesia_maintenance.value(), writer)?;
		}
...
impl WritableSchemaType for AirwayManagementType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("AirwayManagementMethod", self.airway_management_method.value(), writer)?;

		if let Some(ref airway_sub_management_method) = self.airway_sub_management_method {
			write_value("AirwaySubManagementMethod", airway_sub_management_method.value(), writer)?;
		}
...
impl WritableSchemaType for LabDataType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("LabDateTime", &self.lab_date_time.to_rfc3339(), writer)?;
...
impl WritableSchemaType for MedicationType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref dose_start) = self.dose_start {
			write_value("DoseStart", &dose_start.to_rfc3339(), writer)?;
		}

		if let Some(ref dose_end) = self.dose_end {
			write_value("DoseEnd", &dose_end.to_rfc3339(), writer)?;
		}
		...
		if let Some(ref med_concentration_unit) = self.med_concentration_unit {
			write_value("MedConcentrationUnit", med_concentration_unit.value(), writer)?;
		}
...
impl WritableSchemaType for MixtureMedicationType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("MixtureMedicationName", &self.mixture_medication_name, writer)?;
		...
		if let Some(ref mixture_medication_types) = self.mixture_medication_type {
			for mixture_medication_type in mixture_medication_types {
				write_value("MixtureMedicationType", mixture_medication_type.value(), writer)?;
			}
		}
		...
		if let Some(ref mixture_med_concentration) = self.mixture_med_concentration {
			write_value("MixtureMedConcentration", &mixture_med_concentration.to_string(), writer)?;
		}

		if let Some(ref mixture_med_concentration_unit) = self.mixture_med_concentration_unit {
			write_value("MixtureMedConcentrationUnit", mixture_med_concentration_unit.value(), writer)?;
		}
...
impl WritableSchemaType for MonitoringPhysiologicType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref monitoring_time) = self.monitoring_time {
			write_value("MonitoringTime", &monitoring_time.to_rfc3339(), writer)?;
		}
		...
		if let Some(ref monitoring_value_numeric) = self.monitoring_value_numeric {
			write_value("MonitoringValueNumeric", &monitoring_value_numeric.to_string(), writer)?;
		}
...
impl WritableSchemaType for OutputEventType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref output_value_numeric) = self.output_value_numeric {
			write_value("OutputValueNumeric", &output_value_numeric.to_string(), writer)?;
		}
		...
		if let Some(ref output_start_date_time) = self.output_start_date_time {
			write_value("OutputStartDateTime", &output_start_date_time.to_rfc3339(), writer)?;
		}

		if let Some(ref output_end_date_time) = self.output_end_date_time {
			write_value("OutputEndDateTime", &output_end_date_time.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for PostOpType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref post_op_disp_date_time) = self.post_op_disp_date_time {
			write_value("PostOpDispDateTime", &post_op_disp_date_time.to_rfc3339(), writer)?;
		}
		...
		if let Some(ref post_op_discharge_date_time) = self.post_op_discharge_date_time {
			write_value("PostOpDischargeDateTime", &post_op_discharge_date_time.to_rfc3339(), writer)?;
		}

		if let Some(ref length_of_hospital_stay) = self.length_of_hospital_stay {
			write_value("LengthOfHospitalStay", &length_of_hospital_stay.to_string(), writer)?;
		}
...
impl WritableSchemaType for PaymentMethodType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref payment_description_sec) = self.payment_description_sec {
			write_value("PaymentDescriptionSec", payment_description_sec.value(), writer)?;
		}

		if let Some(ref payment_description_th) = self.payment_description_th {
			write_value("PaymentDescriptionTh", payment_description_th.value(), writer)?;
		}
...
impl WritableSchemaType for TimingMilestoneType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("TMStartTime", &self.tm_start_time.to_rfc3339(), writer)?;

		if let Some(ref tm_end_time) = self.tm_end_time {
			write_value("TMEndTime", &tm_end_time.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for ICEventType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref ic_event_time_date) = self.ic_event_time_date {
			write_value("ICEventTimeDate", &ic_event_time_date.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for OutcomeCodeType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("OutcomeOccurred", &self.outcome_occurred.to_string(), writer)?;

		if let Some(ref outcome_time_stamp) = self.outcome_time_stamp {
			write_value("OutcomeTimeStamp", &outcome_time_stamp.to_rfc3339(), writer)?;
		}
...
impl WritableSchemaType for IntakeOutputTotalType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref intake_output_direction) = self.intake_output_direction {
			write_value("IntakeOutputDirection", intake_output_direction.value(), writer)?;
		}
...
impl WritableSchemaType for IntubationDetailsType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref intubation_attempts) = self.intubation_attempts {
			write_value("IntubationAttempts", &intubation_attempts.to_string(), writer)?;
		}
...
impl WritableSchemaType for IntubationDetailsPropertyType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		if let Some(ref intubation_details_property_value) = self.intubation_details_property_value {
			write_value("IntubationDetailsPropertyValue", intubation_details_property_value, writer)?;
		}
...
impl WritableSchemaType for AnesthesiaDetailsDataType {
	fn write<W: Write>(&self, name: &str, writer: &mut EventWriter<W>) -> Result<(), EmitterError> {
		...
		write_value("AnesthesiaDetailsCategory", self.anesthesia_details_category.value(), writer)?;
